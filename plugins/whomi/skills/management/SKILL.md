---
name: management
description: Manage whomi Todos, execution lanes, Codex task bindings, branches, and worktrees. Use when the user asks to capture, queue, start, complete, archive, or inspect a Plan or Todo.
---

# whomi management

Use the whomi MCP tools as the source of truth for task-management operations.
...
1. When the user asks to open, show, view, or manage the visual Plan/Todo UI, use `open_whomi`. The component is interactive; avoid repeating its full contents in prose.
...
6. Distinguish the two execution paths:
   ...
   When a turn begins with a whomi execution marker and instructs you to call `register_current_todo`, do that before editing files. If registration reports a task-binding mismatch, stop and ask the user to bind the Plan to the current task; do not create another task.
